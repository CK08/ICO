function [nhx,Uh,nhy,Vh,Qw] = OrderElevation2D(n,p,U,m,q,V,Pw,dir,t)
...
bpts = zeros(p+1,q+1,4);
ebpts = zeros(p+t+1,q+t+1,4);
Nextbpts = zeros(p-1,q-1,4);
alphas = zeros(p-1,q-1,1);
    ...
    while(b < ii)
        ...
        if(r > 0)
            ...
            for j=1:r
                ...
                for k=p:-1:s
                    for row=1:mc
                        bpts(k+1,row,:)=alfs(k-s+1)*bpts(k+1,row,:)+(1.0-alfs(k-s+1))*bpts(k,row,:);
                    end
                end
                
                for row=1:mc
                    Nextbpts(saved+1,row,:) = bpts(p+1,row,:);
                end
            end
        end
        ...
        for i=lbz:ph
            ...
            for j=max(0,i-t):mpi
                for row=1:mc
                    ebpts(i+1,row,:) = ebpts(i+1,row,:) + bezalfs(i+1,j+1)*bpts(j+1,row,:);
                end
            end
        end
            ...
            for tr=1:oldr-1
                ...
                while j-i > tr
                    if(i < cind)
                        ...
                        for row=1:mc
                            Qw(i+1,row,:) = alf*Qw(i+1,row,:) + (1.0-alf)*Qw(i,row,:);
                        end
                    end
                    ...
                    if j >=lbz
                        if j-tr <= kind-ph+oldr
                            ...
                            for row=1:mc
                                ebpts(kj+1,row,:) = gam*ebpts(kj+1,row,:)+(1.0-gam)*ebpts(kj+2,row,:);
                            end
                        else
                            for row=1:mc
                                ebpts(kj+1,row,:) = bet*ebpts(kj+1,row,:)+(1.0-bet)*ebpts(kj+2,row,:);
                            end
                        end
                    end
        ...
        if b<ii
            for j=0:r-1
                for row=1:mc
                    bpts(j+1,row,:)=Nextbpts(j+1,row,:);
                end
            end
            
            for j=r:p
                for row=1:mc
                    bpts(j+1,row,:)=Pw(b-p+j+1,row,:);
                end
            end
    ...
    while(b < ii)
        ...
        if(r > 0)
            ...
            for j=1:r
                ...
                for k=q:-1:s
                    for row=1:nc
                        bpts(row,k+1,:)=alfs(k-s+1)*bpts(row,k+1,:)+(1.0-alfs(k-s+1))*bpts(row,k,:);
                    end
                end
                
                for row=1:nc
                    Nextbpts(row,saved+1,:) = bpts(row,q+1,:);
                end
            end
        end
        ...
        for i=lbz:ph
            ...
            for j=max(0,i-t):mpi
                for row=1:nc
                    ebpts(row,i+1,:) = ebpts(row,i+1,:) + bezalfs(i+1,j+1)*bpts(row,j+1,:);
                end
            end
        end
            ...
            for tr=1:oldr-1
                ...
                while j-i > tr
                    if(i < cind)
                        ...
                        for row=1:nc
                            Qw(row,i+1,:) = alf*Qw(row,i+1,:) + (1.0-alf)*Qw(row,i,:);
                        end
                    end
                    ...
                    if j >=lbz
                        if j-tr <= kind-ph+oldr
                            ...
                            for row=1:nc
                                ebpts(row,kj+1,:) = gam*ebpts(row,kj+1,:)+(1.0-gam)*ebpts(row,kj+2,:);
                            end
                        else
                            for row=1:nc
                                ebpts(row,kj+1,:) = bet*ebpts(row,kj+1,:)+(1.0-bet)*ebpts(row,kj+2,:);
                            end
                        end
                    end
        ...
        if b<ii
            for j=0:r-1
                for row=1:nc
                    bpts(row,j+1,:)=Nextbpts(row,j+1,:);
                end
            end
            
            for j=r:q
                for row=1:nc
                    bpts(row,j+1,:)=Pw(row,b-q+j+1,:);
                end
            end
...
% bpts = zeros(p+1,q+1,3);
% ebpts = zeros(p+t+1,q+t+1,3);
% Nextbpts = zeros(p-1,q-1,3);
% alphas = zeros(p-1,q-1,1);
% 
% nhx = 0;
% nhy = 0;
