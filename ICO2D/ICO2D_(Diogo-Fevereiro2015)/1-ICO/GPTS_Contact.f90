subroutine GPTS_Contact(iter,inc,istep)
    ...
    !------------------------------------    
    !Added to use ieee_is_nan() function
    ...
    do i=1, ncp_s+p_s
        if(u_knot_slave(i) .ne. u_knot_slave(i+1)) then
            ...
            do iGP=1, SurfGP
                ...
                gaplog = ieee_is_nan (final_gap(ipos)) !Alteracao Diogo
                ...
                if(PTS_active(ipos) == 1)then
                    ...
                    Kgeo = Kgeo + ( gn/m11*matmul(Nbar,transpose(Nbar)) + matmul(matD,transpose(N0s)) + &
                    & matmul(N0s,transpose(matD)) - matmul(matD,transpose(matD))*k11 )*det*wg(iGP)*gn
                    !--------------------------------------------------------------------------------------------------------------------------------------------------------------------
