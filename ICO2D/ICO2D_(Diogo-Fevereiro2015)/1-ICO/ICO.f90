...
!--------------------------------------------------------------------------------------------------    
    ...
    !call ICOHeader()
    call ScreenData(1,inc,iter,AbsRes,absdisp,0)
    ...
    !Read input file
    !------------------------------------------------------------------------------
    call ScreenData(2,inc,iter,AbsRes,absdisp,0)
    
    call ReadInFile(FileName)
    
    call ScreenData(3,inc,iter,AbsRes,absdisp,0)
    !------------------------------------------------------------------------------
    !Data for underformed mesh (Matlab)
    !------------------------------------------------------------------------------
    call MatlabIn()
    
    call ScreenData(4,inc,iter,AbsRes,absdisp,0)
    ...
    do istp=1,nstp
        ...
        call ScreenData(5,inc,iter,AbsRes,absdisp,0)
        ...
        do inc=1,incmax
            ...
            call ScreenData(6,inc,iter,AbsRes,absdisp,0)
...
!                                            call ElQuad9EBBar(nze,Ke,KBar,dispin,Finte)
                ...
                if(ContactPTS == .true. .and. iter .gt. 1)then
                    ...
                    call PTS_ComputeGap_DC(iter,inc,istp,iLM)
...
!                end if
                ...
                ! Convergence Check
                !-----------------------------------------------------------------
                call ConvCheck (inc,iter,AbsRes,AbsDisp) ! ConvCheck foi alterado com o ScreenData -Diogo
                
                call ScreenData(12,inc,iter,AbsRes,absdisp,0)
...
!                    11 format(6(E,1x))
                    ...
                    !FintLM = 0.0d0
                    !Lagrange = 0.0d0
                    call ScreenData(8,inc,iter,AbsRes,absdisp,0)
            ...
            call ScreenData(9,inc,iter,AbsRes,absdisp,0)
            goto 999
        ...
        call ScreenData(10,inc,iter,AbsRes,absdisp,0)
    ...
    !call ScreenData(11,inc,iter,AbsRes,absdisp,0)
    continue
