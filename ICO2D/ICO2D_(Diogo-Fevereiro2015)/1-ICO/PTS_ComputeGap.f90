subroutine PTS_ComputeGap(iter,inc,istep,iLM)
    ...
    !------------------------------------    
    !Added to use ieee_is_nan() function - Diogo 
    ...
    real(8),dimension((ncp_m+ncp_s)*nds,1)::Bxi,Bnxi,Bt,baux1,baux2
    ...
    do i=1,ngrv
        ...
        gaplog = ieee_is_nan (final_gap(i)) !Alteracao Diogo ISNAN()
...
!                    
!            Kdelta = 0.0d0
!            Kdelta = gn/a11*matmul(Nbar,transpose(Nbar)) + matmul(matD,transpose(N0s)) + matmul(N0s,transpose(matD)) - matmul(matD,transpose(matD))*b11
!            Kdelta = Kdelta*Lagrange(i)
!            
!            write(51,*)(ncp_s+ncp_m)*nds
!            do k1=1,(ncp_s+ncp_m)*nds
!                do k2=1,(ncp_s+ncp_m)*nds
!                    write(51,*)kdelta(k1,k2)
!                end do
!            end do
            !-----------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            !------------------------------------------------------------------
            !Assemble slave body contributions
            do k1=1,ncp_s
                do k2=1,ncp_s
            ...
            !Assemble slave and master body contributions
!            do k1=1,ncp_s+ncp_m
!                do k2=1,ncp_s+ncp_m
!                    KLM(smconn(k1)*nds-1,smconn(k2)*nds-1) = KLM(smconn(k1)*nds-1,smconn(k2)*nds-1) + Kdelta(k1*nds-1,k2*nds-1)
!                    KLM(smconn(k1)*nds-1,smconn(k2)*nds  ) = KLM(smconn(k1)*nds-1,smconn(k2)*nds  ) + Kdelta(k1*nds-1,k2*nds  )
...
!            end do
    ...
    if(iLM .gt. 0)then
        ...
        !Write matrices for testing ------------------------------------------------------------------------
        open(unit=51,file='KLM.txt')
       
        write(51,*)tnodes*nds+ngrv
        do k1=1,tnodes*nds+ngrv
            do k2=1,tnodes*nds+ngrv
                write(51,FMT=102)KLM(k1,k2)
            end do
        end do
        close(51)
        
        open(unit=52,file='Kf.txt')
        write(52,*)tnodes*nds
        do k1=1,tnodes*nds
            do k2=1,tnodes*nds
                write(52,FMT=102)Kf(k1,k2)
            end do
        end do
        close(52)
...
!        end do
        ...
        do i=tnodes*nds+1,tnodes*nds+ngrv
            if(ImpDispLM(i,1)/=0.0d0)then
                do k1=tnodes*nds,tnodes*nds+ngrv
                    if(i /= k1) then
...
!    integer(4),intent(OUT)::iLM
!    
!    integer(4),parameter::knewton = 10
!    
!    integer(4)::i,j,k,k1,k2,k3
!    
!    integer(4)::kspan,count
!    integer(4),dimension(ncp_s+ncp_m)::smconn
!    
!    real(8)::xib,lm,xib0,rsd,rsd0,lixo1,lixo2,gn,cfv,a11,b11
!    real(8),dimension(2,1)::xs,xm,vec,tang,nor,vec0,ddxm
!    real(8),dimension(p_s+1)::Rss,dRss,ddRss
!    real(8),dimension(ncp_s)::Rs,dRsdxi,d2Rsdxi2
!    real(8),dimension(p_m+1)::Rmm,dRmm,ddRmm
!    real(8),dimension(ncp_m)::Rm,dRmdxi,d2Rmdxi2
!    
!    real(8),dimension((ncp_s+ncp_m)*nds,1)::Ns,N1,T1,barN,hatN,matD,hatT
!    
!    real(8),dimension((ncp_s+ncp_m)*nds,(ncp_s+ncp_m)*nds)::Kgeo
!    real(8),dimension(tnodes*nds+ngrv,tnodes*nds+ngrv)::KLM
!    real(8),dimension(tnodes*nds,ngrv)::FLM
!    real(8),dimension(tnodes*nds+ngrv,1)::GLM
!    
!    integer(4),dimension(:),allocatable::redcountLM
!    real(8),dimension(:,:),allocatable:: KfeqLM,FextEqLM,dispeqvLM
!                
!    ngrv = ncp_s
!    KLM = 0.0d0
!    FLM = 0.0d0
...
!    do i=1,ngrv
!        
!        !Compute slave B-Spline basis functions...
!        call BSplineBasisAndDeriv2(ncp_s,p_s,grev(i),u_knot_slave,Rss,dRss,ddRss,kspan)
!        
!        !... and convert to NURBS basis functions
!        call BSplinesToNURBS2(ncp_s,p_s,grev(i),u_knot_slave,Rss,dRss,ddRss,kspan,conn_slave,Rs,dRsdxi,d2Rsdxi2)
!        
!        !Compute slave point coordinates
!        xs = 0.0d0
!        do k1=1,ncp_s
!            xs(1,1) = xs(1,1) + Rs(k1)*(GCoords(conn_slave(k1),1) + ddisp(conn_slave(k1)*nds-1,1))
!            xs(2,1) = xs(2,1) + Rs(k1)*(GCoords(conn_slave(k1),2) + ddisp(conn_slave(k1)*nds  ,1))
!        end do
!        
!        !Determine Closest Point Projection
!        xib0 = 0.0d0
!        xib = 0.12345d0
!        do k1 = 1, knewton
!            if(k1==1)then
!                !Compute master B-Spline basis functions at the start of the parametric space...
!                call BSplineBasisAndDeriv2(ncp_m,p_m,xib0,u_knot_master,Rmm,dRmm,ddRmm,kspan)
!                
!                !... and convert to NURBS basis functions
!                call BSplinesToNURBS2(ncp_m,p_m,xib0,u_knot_master,Rmm,dRmm,ddRmm,kspan,conn_master,Rm,dRmdxi,d2Rmdxi2)
!                
!                !Master segment point
!                xm = 0.0d0
!                do k2=1,ncp_m
!                    xm(1,1) = xm(1,1) + Rm(k2)*(GCoords(conn_master(k2),1) + ddisp(conn_master(k2)*nds-1,1))
!                    xm(2,1) = xm(2,1) + Rm(k2)*(GCoords(conn_master(k2),2) + ddisp(conn_master(k2)*nds  ,1))
!                end do
!                
!                !Master segment tangent
!                tang = 0.0d0
!                do k2=1,ncp_m
!                    tang(1,1) = tang(1,1) + dRmdxi(k2)*(GCoords(conn_master(k2),1) + ddisp(conn_master(k2)*nds-1,1))
!                    tang(2,1) = tang(2,1) + dRmdxi(k2)*(GCoords(conn_master(k2),2) + ddisp(conn_master(k2)*nds  ,1))
!                end do
!                
...
!                vec0 = xs-xm
!                
!                rsd0 = vec0(1,1)*tang(1,1) + vec0(2,1)*tang(2,1)
!                
!            end if
!            
!            !Compute master B-Spline basis functions...
!            call BSplineBasisAndDeriv2(ncp_m,p_m,xib,u_knot_master,Rmm,dRmm,ddRmm,kspan)
!            
!            !... and convert to NURBS basis functions
!            call BSplinesToNURBS2(ncp_m,p_m,xib,u_knot_master,Rmm,dRmm,ddRmm,kspan,conn_master,Rm,dRmdxi,d2Rmdxi2)
!            
!            !Master surface point
!            xm = 0.0d0
!            do k2=1,ncp_m
!                xm(1,1) = xm(1,1) + Rm(k2)*(GCoords(conn_master(k2),1) + ddisp(conn_master(k2)*nds-1,1))
!                xm(2,1) = xm(2,1) + Rm(k2)*(GCoords(conn_master(k2),2) + ddisp(conn_master(k2)*nds  ,1))
...
!       1 continue
!       
!       !Compute normal vector to the master surface
!       nor(1,1) = -1.0d0*tang(2,1)
!       nor(2,1) =        tang(1,1)
!       
!       !xs - xm vector
!       vec = xs - xm
...
!        if(PTS_active(i) == 1)then
!            
!            !Compute master B-Spline basis functions at the CPP...
!            call BSplineBasisAndDeriv2(ncp_m,p_m,xib,u_knot_master,Rmm,dRmm,ddRmm,kspan)
!            
!            !... and convert to NURBS basis functions
!            call BSplinesToNURBS2(ncp_m,p_m,xib,u_knot_master,Rmm,dRmm,ddRmm,kspan,conn_master,Rm,dRmdxi,d2Rmdxi2)
!            
!            !Metric of the segment
!            a11 = lm*lm
!            
!            !Curvature of the segment
!            ddxm = 0.0d0
!            do k1=1,ncp_m
!                ddxm(1,1) = ddxm(1,1) + d2Rmdxi2(k1)*(GCoords(conn_master(k1),1) + ddisp(conn_master(k1)*nds-1,1))
!                ddxm(2,1) = ddxm(2,1) + d2Rmdxi2(k1)*(GCoords(conn_master(k1),2) + ddisp(conn_master(k1)*nds  ,1))
!            end do
!            
!            b11 = ddxm(1,1)*nor(1,1) + ddxm(2,1)*nor(2,1)
!            
...
!            
!            do k1=1,ncp_s
!                Ns(k1*nds-1,1) = Rs(k1)*nor(1,1)
!                Ns(k1*nds  ,1) = Rs(k1)*nor(2,1)
!                
!                T1(k1*nds-1,1) = Rs(k1)*tang(1,1)
!                T1(k1*nds  ,1) = Rs(k1)*tang(2,1)
!            end do
!            
!            count = 0
!            do k1=ncp_s+1,ncp_s+ncp_m
!                count = count + 1
!                Ns(k1*nds-1,1) = -1.0d0*Rm(count)*nor(1,1)
!                Ns(k1*nds  ,1) = -1.0d0*Rm(count)*nor(2,1)
!                
!                T1(k1*nds-1,1) = -1.0d0*Rm(count)*tang(1,1)
!                T1(k1*nds  ,1) = -1.0d0*Rm(count)*tang(2,1)
!                
!                N1(k1*nds-1,1) = -1.0d0*dRmdxi(count)*nor(1,1)
!                N1(k1*nds  ,1) = -1.0d0*dRmdxi(count)*nor(2,1)
!            end do
!            
!            !Geometric contribution of contact
!            hatN = N1
!            hatT = T1
!            
!            matD = (hatT-gn*hatN)/(a11-gn*b11)
!            
!            barN = hatN - b11*matD
!            
!            Kgeo = 0.0d0
!            Kgeo = gn/a11*matmul(barN,transpose(barN)) + &
!                 &        matmul(matD,transpose(hatN)) + &
!                 &        matmul(hatN,transpose(matD)) + &
!                 &    b11*matmul(matD,transpose(matD))
!            
!            Kgeo = Kgeo*Lagrange(i)
!            
!            !Slave-master connectivities
!            smconn = 0
!            do k1=1,ncp_s
!                smconn(k1) = conn_slave(k1)
!            end do
!            
!            count = 0
!            do k1=ncp_s+1,ncp_s+ncp_m
!                count = count + 1
!                smconn(k1) = conn_master(count)
!            end do
!            
!            do k1=1,ncp_s+ncp_m
!                
!                do k2=1,ncp_s+ncp_m
!                    KLM(smconn(k1)*nds-1,smconn(k2)*nds-1) = KLM(smconn(k1)*nds-1,smconn(k2)*nds-1) + Kgeo(k1*nds-1,k2*nds-1)
!                    KLM(smconn(k1)*nds  ,smconn(k2)*nds-1) = KLM(smconn(k1)*nds  ,smconn(k2)*nds-1) + Kgeo(k1*nds  ,k2*nds-1)
!                    KLM(smconn(k1)*nds-1,smconn(k2)*nds  ) = KLM(smconn(k1)*nds-1,smconn(k2)*nds  ) + Kgeo(k1*nds-1,k2*nds  )
!                    KLM(smconn(k1)*nds  ,smconn(k2)*nds  ) = KLM(smconn(k1)*nds  ,smconn(k2)*nds  ) + Kgeo(k1*nds  ,k2*nds  )
!                end do
!                
!                KLM(smconn(k1)*nds-1,tnodes*nds+i) = KLM(smconn(k1)*nds-1,tnodes*nds+i) + Ns(k1*nds-1,1)
!                KLM(smconn(k1)*nds  ,tnodes*nds+i) = KLM(smconn(k1)*nds  ,tnodes*nds+i) + Ns(k1*nds  ,1)
...
!            end do
!            
!            GLM(tnodes*nds+i,1) = -1.0d0*gn
!            
!            continue    
!       
!        else !contact status not active
!        
!        !If contact has not occured
!        KLM(tnodes*nds+i,:) = 0.0d0
!        KLM(:,tnodes*nds+i) = 0.0d0
!        KLM(tnodes*nds+i,tnodes*nds+i) = 1.0d0
!        
!        FLM(:,i) = 0.0d0
!        
!        end if !contact status
!       
!    end do !Slave points cycle
!    
...
!    GLM(1:tnodes*nds,1) = GLM(1:tnodes*nds,1) + Fext(1:tnodes*nds,1)
!    
!    allocate(redcountLM(tnodes*nds+ngrv))
!    redcountLM = 1
!    redcountLM(1:tnodes*nds) = redcount(1:tnodes*nds)
!    
!    allocate(KfeqLM(tnodes*nds+ngrv-nBC,tnodes*nds+ngrv-nBC))
!    KfeqLM = 0.0d0
!    
!    allocate(FextEqLM(tnodes*nds+ngrv-nBC,1),dispeqvLM(tnodes*nds+ngrv-nBC,1))
!    FextEqLM = 0.0d0
!    dispeqvLM = 0.0d0
!    
!    !Assembled reduced system of equations
!    k1 = 1
!    k2 = 1
!    do i=1,tnodes*nds+ngrv 
!        do j=1,tnodes*nds+ngrv
!            if (redcountLM(i)/= 0 .and. redcountLM(j)/= 0)then
!                KfeqLM(k1,k2)  = KLM(i,j)
!                FextEqLM(k1,1) = GLM(i,1)                
!                k2=k2+1
!                if(k2==tnodes*nds+ngrv-nbc+1)then
!                    k1=k1+1
!                    k2=1
!                end if
!            end if
...
!    call Gauss (tnodes*nds-nbc+ngrv,KfeqLM,FextEqLM,dispeqvLM)
!    
!    !Assemble increment of the displacement increment
!    j=1
!    ddDisp = 0.0d0
!    do i=1,tnodes*nds
!        if(redcount(i)==1)then
!            ddDisp(i,1)=dispeqvLM(j,1)
!            j=j+1
!        end if
!    end do
!    
...
!    do k1=1,ngrv
!        Lagrange(k1) = Lagrange(k1) + dispeqvLM(tnodes*nds-nBC+k1,1)
!    end do
!    
!    !Assemble Contact internal forces
!    FintLM = 0.0d0
!    do k1=1,ngrv
!        FintLM(:,1) = FintLM(:,1) + FLM(:,k1)*Lagrange(k1)
!    end do
!    
!    continue
!    
