...
subroutine CtPTS(iter,inc,istep,iLM)
    ...
    do i=1,ngrv
        ...
        dtang = 0.0d0
        do k=1,ncp_m
            tang(1,1)  = tang(1,1)  + dRmdxi(k)*(GCoords(conn_master(k),1)&
                         + ddisp(conn_master(k)*2-1,1) - ddDisp(conn_master(k)*2-1,1))
            tang(2,1)  = tang(2,1)  + dRmdxi(k)*(GCoords(conn_master(k),2)&
                         + ddisp(conn_master(k)*2  ,1) - ddDisp(conn_master(k)*2  ,1))
        end do
        ...
        do j=1,knewton
            if(j==1)then
               ...
               !... and convert to NURBS basis, including vanishing terms 
                ...
                dxm = 0.0d0
                do k=1,ncp_m
                    xm(1,1)  = xm(1,1)  + Rm(k)*(GCoords(conn_master(k),1)&
                             + ddisp(conn_master(k)*2-1,1) - ddDisp(conn_master(k)*2-1,1))
                    xm(2,1)  = xm(2,1)  + Rm(k)*(GCoords(conn_master(k),2)&
                             + ddisp(conn_master(k)*2  ,1) - ddDisp(conn_master(k)*2  ,1))
                    continue
                end do
...
!            end if
        ...
        do i=1,ngrv
            ...
            do k1=1,ncp_s
                FintLM(conn_slave(k1)*nds-1,1) = FintLM(conn_slave(k1)*nds-1,1)&
                                         + FLM(conn_slave(k1)*nds-1,i)*dispLM(tnodes*nds-nbc+i,1)
                FintLM(conn_slave(k1)*nds  ,1) = FintLM(conn_slave(k1)*nds  ,1)&
                                         + FLM(conn_slave(k1)*nds  ,i)*dispLM(tnodes*nds-nbc+i,1)
            end do
