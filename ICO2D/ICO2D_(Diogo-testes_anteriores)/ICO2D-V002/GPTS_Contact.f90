subroutine GPTS_Contact(iter,inc,istep)
    ...
    do i=1, ncp_s+p_s
        if(u_knot_slave(i) .ne. u_knot_slave(i+1)) then
            ...
            do iGP=1, SurfGP
                ...
                !-----------------------------
                ! Foi comentado por: Diogo
                ! ISNAN esta a dar problema de compilacao com o IMPLICIT NONE
                !-----------------------------
                ...
                if(PTS_active(ipos) == 1)then
                    ...
                    Kgeo = Kgeo + ( gn/m11*matmul(Nbar,transpose(Nbar)) + matmul(matD,transpose(N0s)) + matmul(N0s,transpose(matD))&
                         - matmul(matD,transpose(matD))*k11 )*det*wg(iGP)
                    !--------------------------------------------------------------------------------------------------------------------------------------------------------------------
                    
                    KPenal = KPenal + en*matmul(Ns,transpose(Ns))*det*wg(iGP)
    ...
    !Assembly contributions to global stiffness matrix
