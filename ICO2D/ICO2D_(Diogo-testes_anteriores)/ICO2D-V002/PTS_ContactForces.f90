...
!    Lagrangey = 0.0d0
    ...
    do i=1,ngrv
        ...
        !if (PTS_active(i)==1)then
            ...
            do k1=1,ncp_s
                FintLM(conn_slave(k1)*nds-1,1) = Fini(conn_slave(k1)*nds-1,1)/&
                        (1.0d0*incmax)*(1.0d0*inc) - Fint(conn_slave(k1)*nds-1,1)
                FintLM(conn_slave(k1)*nds  ,1) = Fini(conn_slave(k1)*nds  ,1)/&
                        (1.0d0*incmax)*(1.0d0*inc) - Fint(conn_slave(k1)*nds  ,1)
            end do
