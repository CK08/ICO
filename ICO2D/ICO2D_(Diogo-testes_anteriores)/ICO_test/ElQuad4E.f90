!------------------------------------------------------------------------------------------------------------
!
! General quadrilateral element for plain strain problems with a 2x2 gaussian quadrature
!
...
subroutine ElQuad4E(nel,nelp,Ke,el_ddisp,Finte,inc)
    ...
    do i=1,npi_xi
        do j=1,npi_eta
            ...
            call MaterialPStrain(iprops,nds,(p+1)*(q+1),props,Bloc_mid,3,el_ddisp,stress(nelp,cpi,:),strain(nelp,cpi,:),&
                                 dstrain(nelp,cpi,:),dstress(nelp,cpi,:),hard(nelp,cpi),matD)
