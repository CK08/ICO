...
subroutine Assembly2D(nze,Ke,finte)
    ...
    if (npenal .gt. 0)then
        ...
        do i=1,npenal
            ...
            Fint(penaldof(i,1)*nds-1,1) = Fint(penaldof(i,1)*nds-1,1) + wp*(dDisp(penaldof(i,1)*nds-1,1)& 
                                        - dDisp(penaldof(i,2)*nds-1,1))
            Fint(penaldof(i,1)*nds  ,1) = Fint(penaldof(i,1)*nds  ,1) + wp*(dDisp(penaldof(i,1)*nds  ,1)& 
                                        - dDisp(penaldof(i,2)*nds  ,1))
            Fint(penaldof(i,2)*nds-1,1) = Fint(penaldof(i,2)*nds-1,1) + wp*(dDisp(penaldof(i,2)*nds-1,1)& 
                                        - dDisp(penaldof(i,1)*nds-1,1))
            Fint(penaldof(i,2)*nds  ,1) = Fint(penaldof(i,2)*nds  ,1) + wp*(dDisp(penaldof(i,2)*nds  ,1)& 
                                        - dDisp(penaldof(i,1)*nds  ,1))
