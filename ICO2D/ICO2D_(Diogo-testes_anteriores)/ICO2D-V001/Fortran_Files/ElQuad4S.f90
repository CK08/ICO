!------------------------------------------------------------------------------------------------------------
!
! General quadrilateral element for plain stress problems with a 2x2 gaussian quadrature
!
...
subroutine ElQuad4S(nel,nelp,Ke,el_ddisp,Finte,inc)
    ...
    integer(4),parameter::npi_xi= 2, npi_eta = 2
    ...
    do i=1,npi_xi
        do j=1,npi_eta
            ...
            call MaterialPStress(iprops,nds,(p+1)*(q+1),props,Bloc_mid,3,el_ddisp,stress(nelp,cpi,:),strain(nelp,cpi,:),dstrain(nelp,cpi,:),dstress(nelp,cpi,:),hard(nelp,cpi),matD)
            ...
            do k1=1,4
                vec = 0.0d0
                if(distload(nel,k1) == 0.0d0)then
                    goto 100
                else
                    if(k1==1)then
                        call ShapeFunc2(nel,xi,-1.0d0,R,dRdx,jac,detj,tzero)
                        vec(2,1) = distload(nel,k1)
                    elseif(k1==2)then
                        call ShapeFunc2(nel,1.0d0,eta,R,dRdx,jac,detj,tzero)
                        vec(1,1) = distload(nel,k1)
                    elseif(k1==3)then
                        call ShapeFunc2(nel,xi,1.0d0,R,dRdx,jac,detj,tzero)
                        vec(2,1) = distload(nel,k1)
                    elseif(k1==4)then
                ...
                dy = 0.0d0
                do k=1,(p+1)*(q+1)
                    dx = dx + dRdx(k,1)*Points(IEN(nel,k),1)
                    dy = dy + dRdx(k,1)*Points(IEN(nel,k),2)
                end do 
                ...
                MatShp = 0.0d0
                do k=1,(p+1)*(q+1)
                    MatShp(1,k*nds-1) = R(k)
                    MatShp(2,k*nds  ) = R(k)
                end do  
                
                fdist = fdist + matmul(transpose(MatShp),vec)*dl*gwt
