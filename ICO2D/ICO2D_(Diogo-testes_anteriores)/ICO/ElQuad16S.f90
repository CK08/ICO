!------------------------------------------------------------------------------------------------------------
!
! General quadrilateral element for plain stress problems with a 4x4 gaussian quadrature
!
...
subroutine ElQuad16S(nel,nelp,Ke,el_ddisp,Finte,inc)
    ...
    integer(4)::i,j,ni,nj,nk,nel_nza,k1,k2
    real(8)::xi,eta,thick
...
!            do k1=1,(p+1)*(q+1)*nds
!                do k2=1,(p+1)*(q+1)*nds
!                    if(ke(k1,k2) .ne. 0.0d0) ke(k1,k2) = 1.0d0
!                end do
!            end do
