...
subroutine ElHex27(nel,nelp,Ke,el_ddisp,Finte,inc)
    
    use Mod_Variables
    implicit none
    
    integer(4)::nel,cpi,count,inc,nelp
    ...
    real(8)::yc,zc,xc
    !real(8), dimension(npi_xi*npi_eta*npi_zeta,nds)::GP_coords
    inodes = (p+1)*(q+1)*(w+1)
    density = props(3)
...
!                end if
                ...
                call gaussCoord(R,el_ddisp,cpi,nelp,npi_xi,npi_eta,npi_zeta,nodes)
!                 ipos = cpi
!                 do k1=1,(p+1)*(q+1)*(w+1)
!                        GP_coords(ipos,1) = GP_coords(ipos,1) + R(k1)*(nodes(k1,1)+el_ddisp(k1*nds-2,1))
!                        GP_coords(ipos,2) = GP_coords(ipos,2) + R(k1)*(nodes(k1,2)+el_ddisp(k1*nds-1,1))
!                        GP_coords(ipos,3) = GP_coords(ipos,3) + R(k1)*(nodes(k1,3)+el_ddisp(k1*nds  ,1))
!                        write(10,FMT=10) nel, cpi, GP_coords(k1,1), GP_coords(k1,2), GP_coords(k1,3)
!                 end do
