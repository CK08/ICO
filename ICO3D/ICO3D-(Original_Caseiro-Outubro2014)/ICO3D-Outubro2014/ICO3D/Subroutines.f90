...
!Subroutine to calculate the cross-product of two vectors
!------------------------------------------------------------------------------------------------------
subroutine CrossProduct(a,b,axb)
 
    implicit none
    integer,parameter :: wp=8
    real(wp),dimension(3,1) :: axb
    real(wp),dimension(3,1),intent(in) :: a
    real(wp),dimension(3,1),intent(in) :: b 
 
    axb(1,1) = a(2,1)*b(3,1) - a(3,1)*b(2,1)
    axb(2,1) = a(3,1)*b(1,1) - a(1,1)*b(3,1)
    axb(3,1) = a(1,1)*b(2,1) - a(2,1)*b(1,1)
