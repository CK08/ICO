!------------------------------------------------------------------------------------------------------------
!
! General first-order hexaedral element with a 3x3x3 gaussian quadrature
!
...
subroutine ElHex8(nel,nzep,Ke,el_ddisp,Finte,inc)
    
    use Mod_Variables
    implicit none
    
    integer(4)::nel,cpi,count,inc,nzep
...
!                    end do
                ...
                call MatPlastic3D(iprops,nds,(p+1)*(q+1)*(w+1),props,Bloc_mid,6,el_ddisp,stress(nzep,cpi,:),&
                        & strain(nzep,cpi,:),dstrain(nzep,cpi,:),dstress(nzep,cpi,:),hard(nzep,cpi),matD)
                ...
                tempstr = 0.0d0
                do k1=1,ntens
                    tempstr(k1,1) = stress(nzep,cpi,k1)
                    deform(k1,1) = dstrain(nzep,cpi,k1)
                end do
