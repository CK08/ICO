...
subroutine ElHex27PVS(nel,Ke,el_ddisp,Finte,inc)
    ...
    real(8),dimension(6,(p+1)*(q+1)*(w+1)*nds)::BDev,BDil,BBar
    ...
    real(8),dimension(6)::temp2
    real(8),dimension(6,6)::TLG
    ...
    real(8),dimension(nds*2,nds*2)::TCL,TGL,TLC
    ...
    do k=1,npi_zeta
        do j=1,npi_eta
            do i=1,npi_xi
                ...
                do k1=1,(p+1)*(q+1)*(w+1)
                    do k2 =1,nds
                        BConv_mid(1,(k1-1)*3+k2) = dRdxii_mid(k1,1)*jac_mid(1,k2)
                        BConv_mid(2,(k1-1)*3+k2) = dRdxii_mid(k1,2)*jac_mid(2,k2)
                        BConv_mid(3,(k1-1)*3+k2) = dRdxii_mid(k1,3)*jac_mid(3,k2)
                        
                        BConv_mid(4,(k1-1)*3+k2) = dRdxii_mid(k1,1)*jac_mid(2,k2) + dRdxii_mid(k1,2)*jac_mid(1,k2)
                        BConv_mid(5,(k1-1)*3+k2) = dRdxii_mid(k1,1)*jac_mid(3,k2) + dRdxii_mid(k1,3)*jac_mid(1,k2)
                        BConv_mid(6,(k1-1)*3+k2) = dRdxii_mid(k1,2)*jac_mid(3,k2) + dRdxii_mid(k1,3)*jac_mid(2,k2)
                    end do
                end do
                
                
                !-----------------------------------------------------------------------------------------------
                ! ANS methodology (mid-configuration)-----------------------------------------------------------
                BNG = BConv_mid
                ...
                do k1=1,6
                    call ShapeFunc3(nel,TyPt(k1,1),TyPt(k1,2),TyPt(k1,3),RA,dRAdx,dRAdxi,dRAdxii,detjA,jacA,dxdxiA,mdisp)
                    jacA = transpose(jacA)
                    ...
                    do k2=1,(p+1)*(q+1)*(w+1)
                        do k3 =1,nds
                            ...
                            & dRAdxii(k2,3)*jacA(1,k3))*MMult(1,k1)
                        end do
                    end do
                ...
                do k1=1,6
                    call ShapeFunc3(nel,TyPt(k1+6,1),TyPt(k1+6,2),TyPt(k1+6,3),RA,dRAdx,dRAdxi,dRAdxii,detjA,jacA,dxdxiA,mdisp)
                    jacA = transpose(jacA)
                    ...
                    do k2=1,(p+1)*(q+1)*(w+1)
                        do k3 =1,nds
                            ...
                            BNG(6,(k2-1)*3+k3) = BNG(6,(k2-1)*3+k3) + (dRAdxii(k2,2)*jacA(3,k3) + &
                                & dRAdxii(k2,3)*jacA(2,k3))*MMult(1,k1)
                        end do
                    end do
                end do
                ...
                do k1=1,4
                    call ShapeFunc3(nel,TyPt(k1+12,1),TyPt(k1+12,2),TyPt(k1+12,3),RA,dRAdx,dRAdxi,dRAdxii,detjA,jacA,dxdxiA,mdisp)
                    jacA = transpose(jacA)
                    
                    do k2=1,(p+1)*(q+1)*(w+1)
                        do k3 =1,nds
                            BNG(4,(k2-1)*3+k3) = BNG(4,(k2-1)*3+k3) + (dRAdxii(k2,1)*jacA(2,k3) + &
                                & dRAdxii(k2,2)*jacA(1,k3))*MMultr(1,k1)
                        end do
                    end do
                end do
                ...
                do k1=1,4
                    ...
                    jacA = transpose(jacA)
                    do k2=1,(p+1)*(q+1)*(w+1)
                        BBar(1,k2*3-2) = BBar(1,k2*3-2) + dRAdx(k2,1)*MMultr(1,k1)
                        BBar(2,k2*3-2) = BBar(2,k2*3-2) + dRAdx(k2,1)*MMultr(1,k1)
                        BBar(3,k2*3-2) = BBar(3,k2*3-2) + dRAdx(k2,1)*MMultr(1,k1)
                        BBar(1,k2*3-1) = BBar(1,k2*3-1) + dRAdx(k2,2)*MMultr(1,k1)
                        BBar(2,k2*3-1) = BBar(2,k2*3-1) + dRAdx(k2,2)*MMultr(1,k1)
                        BBar(3,k2*3-1) = BBar(3,k2*3-1) + dRAdx(k2,2)*MMultr(1,k1)
                        BBar(1,k2*3  ) = BBar(1,k2*3  ) + dRAdx(k2,3)*MMultr(1,k1)
                        BBar(2,k2*3  ) = BBar(2,k2*3  ) + dRAdx(k2,3)*MMultr(1,k1)
                        BBar(3,k2*3  ) = BBar(3,k2*3  ) + dRAdx(k2,3)*MMultr(1,k1)
                    end do
                    
                end do
                
                BBar = BBar/3.0d0
                !BBar = matmul(TGL,BBar)
                !BBar = matmul(TLC,BBar)
                
                Bdev = 0.0d0
                Bdev  = Bglob - Bdil
                
                !Bglob(1,:) = Bdev(1,:) + BBar(1,:)
                !Bglob(2,:) = Bdev(2,:) + BBar(2,:)
                !Bglob(3,:) = Bdev(3,:) + BBar(3,:)
                Bglob = Bdev + BBar
                
                Bglob = matmul(TGL,Bglob)
                BConv = matmul(TLC,Bglob)
...
!                end do
                ...
                do k1=1,6
                    call ShapeFunc3(nel,TyPt(k1+6,1),TyPt(k1+6,2),TyPt(k1+6,3),RA,dRAdx,dRAdxi,dRAdxii,detjA,jacA,dxdxiA,tdisp)
                    jacA = transpose(jacA)
