...
!                    do k3=0,p
!                        count = count + 1
!                        MTP(k1,count) = Nb(p+1-k3)
!                    end do      
!                end do
!                
!                temp3 = 0.0d0
!                call gaussj(MTP,3,temp3,ilixo)
!                continue
!                
!                
!                !IP Mass Matrix ----------------------------------------------------
...
!                    
!                    do k2=1,(p+1)*(q+1)*(w+1)
!                        BNG(1,k2*3-2) = BNG(1,k2*3-2) + dRAdx(k2,1)*MMult(1,k1)
!                        BNG(1,k2*3-1) = BNG(1,k2*3-1) + dRAdx(k2,2)*MMult(1,k1)
!                        BNG(1,k2*3  ) = BNG(1,k2*3  ) + dRAdx(k2,3)*MMult(1,k1)
!                    end do
!                end do
!                !-------------------------------------------------------------------------------------------
!                !-------------------------------------------------------------------------------------------
...
!                
!                do k1=1,3
!                    call ShapeFunc3(nel,TyPt(k1+3,1),TyPt(k1+3,2),TyPt(k1+3,3),RA,dRAdx,dRAdxi,dRAdxii,detjA,jacA,dxdxiA,tdisp)
!                    jacA = transpose(jacA)
!                    
!                    do k2=1,(p+1)*(q+1)*(w+1)
!                        BNG(2,k2*3-2) = BNG(2,k2*3-2) + dRAdx(k2,1)*MMult(1,k1)
!                        BNG(2,k2*3-1) = BNG(2,k2*3-1) + dRAdx(k2,2)*MMult(1,k1)
!                        BNG(2,k2*3  ) = BNG(2,k2*3  ) + dRAdx(k2,3)*MMult(1,k1)
!                    end do
!                end do
!                !-------------------------------------------------------------------------------------------
!                !-------------------------------------------------------------------------------------------
...
!                MTP = 0.0d0
!                do k1=1,3
!                    wk = (TyPt(k1+6,3) + 1.0d0)/2.0d0
!                    
!                    Lb = 0.0d0
!                    call BSplineBasisAndDeriv(ncpz*w,w  ,wk, wb, Lb,dLbdzeta)
!                    
!                    count = 0
!                    do k3=0,w
!                        count = count + 1
!                        MTP(k1,count) = LB(w+1-k3)
!                    end do      
!                end do
!                
!                temp3 = 0.0d0
...
!                    
!                    do k2=1,(p+1)*(q+1)*(w+1)
!                        BNG(3,k2*3-2) = BNG(3,k2*3-2) + dRAdx(k2,1)*MMult(1,k1)
!                        BNG(3,k2*3-1) = BNG(3,k2*3-1) + dRAdx(k2,2)*MMult(1,k1)
!                        BNG(3,k2*3  ) = BNG(3,k2*3  ) + dRAdx(k2,3)*MMult(1,k1)
!                    end do
!                end do
!                !-------------------------------------------------------------------------------------------
!                !-------------------------------------------------------------------------------------------
...
!                TyPt = 0.0d0
!                
!                TyPt(1,1) =  CA; TyPt(1,2) =  CA; TyPt(1,3) =  zeta
!                TyPt(2,1) =  CA; TyPt(2,2) = -CA; TyPt(2,3) =  zeta
!                TyPt(3,1) = -CA; TyPt(3,2) =  CA; TyPt(3,3) =  zeta
!                TyPt(4,1) = -CA; TyPt(4,2) = -CA; TyPt(4,3) =  zeta
!                
!                TyPt(5,2) =  CA; TyPt(5,3) =  CA; TyPt(5,1) =  xi
!                TyPt(6,2) =  CA; TyPt(6,3) = -CA; TyPt(6,1) =  xi
!                TyPt(7,2) = -CA; TyPt(7,3) =  CA; TyPt(7,1) =  xi
!                TyPt(8,2) = -CA; TyPt(8,3) = -CA; TyPt(8,1) =  xi
!                
!                TyPt( 9,1) =  CA; TyPt( 9,3) =  CA; TyPt( 9,2) =  eta
!                TyPt(10,1) =  CA; TyPt(10,3) = -CA; TyPt(10,2) =  eta
!                TyPt(11,1) = -CA; TyPt(11,3) =  CA; TyPt(11,2) =  eta
!                TyPt(12,1) = -CA; TyPt(12,3) = -CA; TyPt(12,2) =  eta
!                
!                ni = INN(IEN(nel,1),1)
!                nj = INN(IEN(nel,1),2)
!                nk = INN(IEN(nel,1),3)
...
!                MTPr = 0.0d0
!                do k1=1,4
!                    uk = (TyPt(k1+4,1) + 1.0d0)/2.0d0
!                    vk = (TyPt(k1+4,2) + 1.0d0)/2.0d0
!                    wk = (TyPt(k1+4,3) + 1.0d0)/2.0d0
!                    
!                    Nbr = 0.0d0
!                    call BSplineBasisAndDeriv(ncpx+2*p,p-1,uk,ubr, Nbr, dNbrdxi)
!                    Mbr = 0.0d0
...
!                        do k3=0,q-1
!                            count = count + 1
!                            MTPr(k1,count) = Lbr(w-k2)*Mbr(q-k3)
!                        end do
!                    end do      
!                end do
!                
...
!                
!                do k1=1,4
!                    call ShapeFunc3(nel,TyPt(k1+4,1),TyPt(k1+4,2),TyPt(k1+4,3),RA,dRAdx,dRAdxi,dRAdxii,detjA,jacA,dxdxiA,tdisp)
!                    jacA = transpose(jacA)
!                    
!                    do k2=1,(p+1)*(q+1)*(w+1)
!                        BNG(1,k2*3-2) = BNG(1,k2*3-2) + dRAdx(k2,1)*MMultr(1,k1)
...
!                MTPr = 0.0d0
!                do k1=1,4
!                    uk = (TyPt(k1+8,1) + 1.0d0)/2.0d0
!                    vk = (TyPt(k1+8,2) + 1.0d0)/2.0d0
!                    wk = (TyPt(k1+8,3) + 1.0d0)/2.0d0
!                    
!                    Nbr = 0.0d0
!                    call BSplineBasisAndDeriv(ncpx+2*p,p-1,uk,ubr, Nbr, dNbrdxi)
!                    Mbr = 0.0d0
...
!                        do k3=0,w-1
!                            count = count + 1
!                            MTPr(k1,count) = Lbr(w-k3)*Nbr(p-k2)
!                        end do
!                    end do      
!                end do
!                
...
!                
!                do k1=1,4
!                    call ShapeFunc3(nel,TyPt(k1+8,1),TyPt(k1+8,2),TyPt(k1+8,3),RA,dRAdx,dRAdxi,dRAdxii,detjA,jacA,dxdxiA,tdisp)
!                    jacA = transpose(jacA)
!                    
!                    do k2=1,(p+1)*(q+1)*(w+1)
!                        BNG(2,k2*3-2) = BNG(2,k2*3-2) + dRAdx(k2,1)*MMultr(1,k1)
