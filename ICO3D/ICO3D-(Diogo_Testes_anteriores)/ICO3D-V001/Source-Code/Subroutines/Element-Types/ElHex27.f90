...
subroutine ElHex27(nel,Ke,el_ddisp,Finte,inc)
    ...
    ! Aloc I/O variables (+ 2 extra - cpi and count)
    ...
    ! Gauss weigths for the local directions (xi, eta, zeta)    
    we = 1.0d0
    wn = 1.0d0
    wc = 1.0d0
    !-------------------------------------------------
    ! Calculate the Gauss points and weigths - Diogo !
    !-------------------------------------------------
    ...
    !##### GNL True
    if(nlgeom==.true.)then
        tdisp = el_ddisp        ! tdisp: Total displacement = element delta displacement
        mdisp = el_ddisp/2.0d0  ! mdisp: Middle displacement = half of element delta displacement
    else ! GNL False
        tdisp = 0.0d0
        mdisp= 0.0d0
        
        TGL = 0.0d0 ! 6x6 Transformation matrix between local and global axis (Do not confuse with Jacobian: natural and global) 
        do i=1,6
            TGL(i,i) = 1.0d0
        end do
    end if
    ...
    !Gauss points cycle
    Ke = 0.0d0   ! Elemental stiffeness matrix     
    KNLG = 0.0d0 ! Non-linear geometric elemental stiffeness matrix
    BNL = 0.0d0  ! Non-linear B matrix 
    MSTR = 0.0d0 ! Matrix with stress components for GNL 
    
    Finte = 0.0d0
    !-----------------------------------------------------
    ! Cycle for each Gauss point in the hexaedral element! 
    !-----------------------------------------------------
    do i=1,npi_xi
        do j=1,npi_eta
            do k=1,npi_zeta
            
                cpi = cpi + 1 ! Counter for the number of points 
                MatD(:,:) = TmatD(nel,cpi,:,:) ! TmatD is the Backup of the D matrix (Declared in Mod_Variables, Allocated in ReadInFile)
                ! Current Gauss point 
                xi = e(i)
                eta = n(j)
                zeta = c(k)
