...
subroutine Assembly3D(nze,Ke,finte)
    ...
    do k1=1,(p+1)*(q+1)*(w+1)
        fint(conn(nze,k1)*3-2,1) = fint(conn(nze,k1)*3-2,1) + finte(k1*3-2,1);
        fint(conn(nze,k1)*3-1,1) = fint(conn(nze,k1)*3-1,1) + finte(k1*3-1,1);
        fint(conn(nze,k1)*3  ,1) = fint(conn(nze,k1)*3  ,1) + finte(k1*3  ,1);
    end do
!   
    continue
     !-----------------------------------------------
!    ! Assemble penalty
     ! Penalty Method for DupNode [Duplicated Nodes]
     ! 
     ! Edited by DC, March 2014.
     !-----------------------------------------------           
!    count=0
!    if (npenal .gt. 0)then
