!----------------------------------------------------------------------------------------------
!
! Subroutine to calculate the deformation gradient matF of 3D problems
!
...
!
!----------------------------------------------------------------------------------------------
  subroutine DefGrad3D(nnodes,ndof,dNdc,dNde,dNdz,jacinv,coords,disp,MatF)
    implicit none
    integer*4,intent(IN)::nnodes,ndof
    real*8,dimension(nnodes),intent(IN)::dNdc,dNde,dNdz
    ...
    do j=1,nnodes
        ...
        do k=1,ndof
            do l=1,ndof
                matF(k,l) = matF(k,l) + dNdxy(l,1)*(coords(j,k) + disp(j*3-(3-k),1))
            end do
        end do
