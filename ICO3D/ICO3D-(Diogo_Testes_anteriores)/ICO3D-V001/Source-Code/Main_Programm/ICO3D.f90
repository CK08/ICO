    ...
    !-----------------------------------------------
    ! O modulo IFPORT nao funciona em MAC OSX: Diogo 
    !-----------------------------------------------
    !use ifport 
    use Mod_Variables
...
!    integer(4)::mnz,countx,county
    ...
    !--------------------------------------------------------------------
    ! Variaveis para o metodo B-Bar implementado pelo J.F. Caseiro: Diogo 
    !--------------------------------------------------------------------
...
!    real(4),dimension(2)::atime
    ...
    !FileName = 'ICOfile.txt'
    !--------------------------------------
    ! Pre-build test files for ICO3D: Diogo
    !--------------------------------------
    ...
    !        - iteration and increment maximum;
    !        - geometric nonlinearity;
    !        - eigen calculation;
    !        - and Gauss solver (1).
    ! Commented by Diogo Cardoso, March 2014.
    !------------------------------------------    
    itermax = 25
    incmax = 1
    nlgeom=.false.
...
!    enddo
    ...
    !--------------------------------------------------------------------------------------------------------
    ! Block {B1.2} - Build connectivities and allocate global arrays                                        !  
    !                                                                                                       !
    ! Commented by Diogo Cardoso, March 2014.                                                               !
    !--------------------------------------------------------------------------------------------------------
    !--------------------------------------------------------------------------------------------------------
    ! Memory alloc of variables:                                                                            !    
    !        - Ke (elemental stiffeness matrix)                                                             !   
    !        - dispin (elemental displacement)                                                              !  
    !        - Finte (Internal Force of the element)                                                        !
    !        - coordi (Initial coordenates - for GNL [Geometric Non-Linearity] calculations)                !
    !--------------------------------------------------------------------------------------------------------
    ...
    !coordi = 0.0d0                                                                                         !
                                                                                                            !
    !--------------------------------------------------------------------------------------------------------
    ! For the B-Bar Method it's necessary to alloc the following variables:                                 !       
    !        - MatA ()                                                                                      !  
    !        - MatC ()                                                                                      !
    !        - MatV ()                                                                                      !
    !        - Temp1 ()                                                                                     !
...
!    call GiDEqMesh(coordi)                                                                                 !
    ...
    Fini = Fext                                                                                             !
    Fext = 0.0d0                                                                                            !
    Finc=Fini/(1.0d0*incmax) ! Multiplication with 1.0d0 is for the division result in a real               !
    SEnergyConv = 0.0d0                                                                                     !
    !--------------------------------------------------------------------------------------------------------
    !--------------------------------------------------------------------------------------------------------
    write(*,*)'...'
    ...
    do inc=1,incmax
        ...
        !-------------------------------------------------------
        ! Block {B2} - Add load increment (Fext) and set du = 0!
        !                                                      !
        ! Commented by Diogo Cardoso, March 2014.              !
        !-------------------------------------------------------
        stpfrac=0.0d0 ! Fraction of the converged displacement !
        dDisp=0.0d0 !                                          !
        ddDisp=0.0d0 !                                         !
                                                               !
        SEnergy = SEnergyConv                                  !
        ...
        do while(iter <= itermax) 
            ...
            !--------------------------------------------------------------
            ! Block {B3} - Get converged variables (stress, strain, etc.) !
            ! Use only converged variables in the iteration cycle         !
            !                                                             !
            ! Edited by Diogo Cardoso, March 2014.                        ! 
            !--------------------------------------------------------------
            Stress = Stress_Conv ! Declared in ReadInFile                 ! 
            Strain = Strain_Conv ! Declared in ReadInFile                 !
            hard = hard_conv     ! Declared in ReadInFile                 !
            laxis = laxisconv    ! Declared in ReadInFile                 !
            SEnergy = SEnergyConv                                         !
            !alpha=alphaconv                                              !
            !StoreC = StoreCConv                                          !
            !--------------------------------------------------------------
            ...
            !Elastic Step ---------------------------------------------
            !-----------------------------------------------------------------------------------------------------------------------
            ! Special Cycle {SC}: Runs in the first iteration of the first increment so that it computes the                       !
            !                     element stiffness and assembles the global arrays of the problem.                                !
            !                                                                                                                      !
            ! commented by Diogo Cardoso, March 2014.                                                                              !
            !-----------------------------------------------------------------------------------------------------------------------
            if(inc == 1 .and. iter == 1)then                                                                                       !
                NZE = 0 !Non-Zero Element (Number of values different than zero in the knot vectores)                              !
                do i=1, ncpx+p                                                                                                     !
                    do j=1,ncpy+q                                                                                                  !
                        do k=1,ncpz+w                                                                                              !
                                                                                                                                   !
                        ...
                        if((u_knot(i) .ne. u_knot(i+1)) .and. (v_knot(j) .ne. v_knot(j+1)) .and. (w_knot(k) .ne. w_knot(k+1))) then!
                                     ! |-> .ne. : (N)ot (E)qual                                                                    ! 
                            nze = nze + 1 !Non-Zero Element increment (Number of values different than zero in the knot vectores)  !
                                                                                                                                   !
                            !-------------------------------------------------------------------------------------------------------
                            ! Alguns casos (de elementos) estao comentados. Foram teste do J.F. Caseiro: Diogo                     !
                            !        PS: As SUBROTINAS existem na pasta do projecto.                                               !
                            !        PS2: Os casos comentados com !** sao comentados por mim (falta verificar                      !
                            !             as dependencias desses elementos).                                                       !
                            !-------------------------------------------------------------------------------------------------------
                            SelectCase(Elcode)                                                                                     !
!**                                case('Hex8')                                                                                    !
...
!                                call AssemblyANS(nze,MatA,MatC,MatV)                                                              !
               ...
               !SEnergy = 0.0d0 ! This declaration has, presumably, no effect on the annalysis since SEnergy was already declared  !
                                ! zero in the beginning of the increment cycle                                                     !
                                                                                                                                   !
            end if !Elastic Step                                                                                                   !
            !-----------------------------------------------------------------------------------------------------------------------
            !-----------------------------------------------------------------------------------------------------------------------
...
!            end do
            ...
            if(elcode=='Hex27BBar' .or. elcode=='Hex8BBar')then
                ...
                do i=1,ncpx*ncpy*ncpz*nds ! Cycle that runs through the DOF in each Control Point 
...
!            end if
            ...
            !---------------------------------------------------------------------------------------------------------------
            ! Block {B6} - Impose prescribed displacements BC's                                                            !
            !                                                                                                              !  
            ! Commented by Diogo Cardoso, March 2014.                                                                      !                  
            !---------------------------------------------------------------------------------------------------------------
            ...
            !---------------------------------------------------------------------------
            ! Block {B7} - Solve system of equations                                   !
            !                                                                          !
            ! Edited by Diogo Cardoso, March 2014.                                     !   
            !---------------------------------------------------------------------------
            if(isolver==1)then                                                         !
                ...
                continue                                                               !         
            !---------------------------------------------------------------------------
            ! A block of code for 2 different solvers existed in this location.        !
            ! A copy of this code exists as backup.                                    !
            !---------------------------------------------------------------------------
            end if                                                                     !
                                                                                       !
            j=1                                                                        !
            ...
            !---------------------------------------------------------------------------------------------------------------------------
            ! Element Cycle {Cele} with blocks {B8} & {B9} - Computes element stiffeness and contributions, and assembles              !  
            !                                                it to global arrays.                                                      !
            !                                                                                                                          !
            ! Commented by Diogo Cardoso, March 2014.                                                                                  !       
            !---------------------------------------------------------------------------------------------------------------------------
                ...
                do j=1,ncpy+q
                    do k=1,ncpz+w
                        ...
                        if((u_knot(i) .ne. u_knot(i+1)) .and. (v_knot(j) .ne. v_knot(j+1)) .and. (w_knot(k) .ne. w_knot(k+1))) then
                            ...
                            do k1=1,(p+1)*(q+1)*(w+1)
                                !dispin is the element displacement with size ((p+1)*(q+1)*(w+1)*nds,1)    
                            ...
                            ! Alguns casos (de elementos) estao comentados. Foram teste do J.F. Caseiro: Diogo
                            !        PS: As SUBROTINAS existem na pasta do projecto.
                            !        PS2: Os casos comentados com !** sao comentados por DC (falta verificar
                            !             as dependencias desses elementos).             
                            !---------------------------------------------------------------------------------
...
!                                call AssemblyANS(nze,MatA,MatC,MatV)
            ...
            ! This fictional step only runs on the first (1º) iteration of the first (1º) increment !
            ! and if, and only if, GNL (Geometric Non-Linearities) is TRUE.                         !
            !                                                                                       !  
            ! Edited by Diogo Cardoso, March 2014.                                                  !
            !----------------------------------------------------------------------------------------
            ...
            !--------------------------------------
            ! Print Residual of the iteration     !
            !                                     !   
            ! Added by Diogo Cardoso, March 2014. !
            !--------------------------------------
            write(*,*)''
            write(*,*)' ! Solution at iteration', iter
            write(*,FMT=15)AbsRes
            write(*,FMT=16)AbsDisp
            15 format('  -> The absolute residual of the forces is R=        ', E)
            16 format('  -> The absolute residual of the displacement is Rd= ', E)
            write(*,*)''
        ...
        !Label descomentada para compilacao do goto 80: Diogo   
         80 continue
...
!    !------------------------------------------------------------
!    ! Faz parte do modulo IFPORT (nao funciona em MAC OSX): Diogo 
!    !------------------------------------------------------------
!    CPUt=etime(atime)
