...
!    real(8),dimension(2,2,2,nds+1),intent(IN)::b_net
!    
...
!    xi   = ((u_knot(ni) - u_knot(ni-1))*xii   + (u_knot(ni) + u_knot(ni-1)))/2.0d0
!    eta  = ((v_knot(nj) - v_knot(nj-1))*etai  + (v_knot(nj) + v_knot(nj-1)))/2.0d0
!    zeta = ((w_knot(nk) - w_knot(nk-1))*zetai + (w_knot(nk) + w_knot(nk-1)))/2.0d0
!    
!    !Determinant of the jacobian parametric-parent
!    detj1 = (U_knot(ni)-U_knot(ni-1))*(V_knot(nj)-V_knot(nj-1))*(W_knot(nk)-W_knot(nk-1))/8.0d0
!    
...
!    
!!    open(unit=999,file='BasisBar.txt',access='append')
!!    write(999,*)N(1),N(2)
!!    close(999)
!    
