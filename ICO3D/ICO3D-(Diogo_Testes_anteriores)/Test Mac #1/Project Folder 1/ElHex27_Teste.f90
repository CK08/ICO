subroutine ElHex27_Teste(nel,Ke,el_ddisp,Finte,inc)
    ...
    real(8),dimension(6,(p+1)*(q+1)*(w+1)*nds)::B0
    real(8),dimension(6,(p+1)*(q+1)*(w+1)*nds)::BDil,BDev
    real(8),dimension(3,3)::jac0
    real(8)::detJ0
    ...
    call ShapeFuncEAS(nel,nnodes,nds,nelems,nshpl,p,q,w,xi,eta,zeta,R0,dRdx0,detj0,jac0,tzero,Points,IEN)
    
    B0 = 0.0d0
    do k1=1,(p+1)*(q+1)*(w+1)
        B0(1,k1*3-2) = dRdx0(k1,1)/3.0d0
        B0(2,k1*3-1) = dRdx0(k1,2)/3.0d0
        B0(3,k1*3  ) = dRdx0(k1,3)/3.0d0
                   
!        B0(4,k1*3-2) = dRdx0(k1,2)
!        B0(4,k1*3-1) = dRdx0(k1,1)
!        B0(5,k1*3-2) = dRdx0(k1,3)
!        B0(5,k1*3  ) = dRdx0(k1,1)
!        B0(6,k1*3-1) = dRdx0(k1,3)
!        B0(6,k1*3  ) = dRdx0(k1,2)         
    end do
