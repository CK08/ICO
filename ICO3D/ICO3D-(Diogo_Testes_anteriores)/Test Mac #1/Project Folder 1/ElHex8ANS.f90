...
subroutine ElHex8ANS(nel,Ke,el_ddisp,Finte,inc)
    ...
    real(8),dimension(6,(p+1)*(q+1)*(w+1)*nds)::Bglob,Bloc,Bloc2
    ...
    real(8),dimension(6,(p+1)*(q+1)*(w+1)*nds)::BNat,BNat2,BTemp
...
!                    end do
                ...
                !Point A --------------------------------------------------------------------------------
                call ShapeFunc3(nel,1.0d0,0.0d0,0.0d0,RA,dRdxA,dRdxiA,dRdxiiA,detjA,jacA,dxdxiA,tzero)
                
                BNat2 = 0.0d0
                do k1=1,(p+1)*(q+1)*(w+1)
                    BNat2(1,k1*3-2) = dRdxA(k1,1)
                    BNat2(2,k1*3-1) = dRdxA(k1,2)
                    BNat2(3,k1*3  ) = dRdxA(k1,3)
                    
                    BNat2(4,k1*3-2) = dRdxA(k1,2)
                    BNat2(4,k1*3-1) = dRdxA(k1,1)
                    BNat2(5,k1*3-2) = dRdxA(k1,3)
                    BNat2(5,k1*3  ) = dRdxA(k1,1)
                    BNat2(6,k1*3-1) = dRdxA(k1,3)
                    BNat2(6,k1*3  ) = dRdxA(k1,2)
                end do
                
                BNat2 = matmul(TGC,BNat2)
                
                BNat(5,:) = BNat(5,:) + 1.0d0/4.0d0*(1.0d0+xi)*BNat2(5,:)
                !----------------------------------------------------------------------------------------
                ...
                !Point B --------------------------------------------------------------------------------
                call ShapeFunc3(nel,0.0d0,1.0d0,0.0d0,RA,dRdxA,dRdxiA,dRdxiiA,detjA,jacA,dxdxiA,tzero)
                ...
                BNat(6,:) = BNat(6,:) + 1.0d0/4.0d0*(1.0d0+eta)*BNat2(6,:)
                !----------------------------------------------------------------------------------------
