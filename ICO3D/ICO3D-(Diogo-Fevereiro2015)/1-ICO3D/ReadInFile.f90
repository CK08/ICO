...
!tem de ser corrigido para o caso com interior knots repetidos            
nelems = (ncpx-p)*(ncpy-q)*(ncpz-w)  !Total number of elements
        ...
        elseif(line=='*beginMP')then
            ...
            nshpl = 0
            do i=1,nptch
                nshpl = nshpl + (MP_p(i)+1)*(MP_q(i)+1)*(MP_w(i)+1)            !Number of local shape functions
            end do                    
        ...
        elseif(line=='*element')then
            ...
            allocate(GP_coords(npi*nelems,nds))
            GP_coords=0.0d0
